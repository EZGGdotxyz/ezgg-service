...
const USDC_ADDRESS: Map<number, string> = new Map()
// baseSepolia
USDC_ADDRESS.set(84532, "0x036CbD53842c5426634e7929541eC2318f3dCF7e");
// polygonAmoy
USDC_ADDRESS.set(80002, "0x41e94eb019c0762f9bfcf9fb1e58725bfb0e7582");
// bscTestnet
USDC_ADDRESS.set(97, "0x64544969ed7EBf5f083679233325356EbE738930");
// arbitrumSepolia
USDC_ADDRESS.set(421614, "0x75faf114eafb1BDbe2F0316DF893fd58CE46AA4d");
// monadTestnet
USDC_ADDRESS.set(10143, "0x0");
// scrollSepolia
USDC_ADDRESS.set(534351, "0x0");
...
export default function DashboardPage2() {
  ...
  useEffect(() => {
    const chain = chains.find((x) => x.id == currentChainId);
    ...
    if (embeddedWallet) {
        (async () => {            
            ...
            const nexusClient = createSmartAccountClient({ 
                ...
                transport: http("https://bundler.biconomy.io/api/v3/97/nJPK7B3ru.dd7f7861-190d-41bd-af80-6877f74b8f44"),
      ...
      <main className="flex flex-col min-h-screen px-4 sm:px-20 py-6 sm:py-10 bg-privy-light-blue">
        ...
        <>
          ...
          <div className="mt-12 flex gap-4 flex-wrap">
            ...
            <Balance chainId={currentChainId} externalAddress={user.wallet.address} smartWalletAddress={user.smartWallet.address}></Balance>
...
function Balance({chainId, externalAddress, smartWalletAddress}:{chainId: number, externalAddress:string, smartWalletAddress:string}) {
    ...
    try {
      ...
      const [externalBalance, smartBalance] = await Promise.all([
        publicClient.readContract({
          address: getAddress(USDC_ADDRESS.get(chainId!)!),
          abi: erc20Abi,
  ...
  useEffect(() => {
    fetchBalances(externalAddress, smartWalletAddress);
  }, [showModal, chainId, externalAddress, smartWalletAddress]);
  ...
  const onWithdraw = async () => {
    try {      
      ...
      await smartWalletClient!.sendTransaction({
        to: getAddress(USDC_ADDRESS.get(chainId!)!),
        data: encodeFunctionData({
          abi: erc20Abi,
          functionName: "transfer",
          args: [getAddress(externalAddress), amountInWei]
        })
      });
  ...
  const handleTransfer = async () => {
    ...
    const {data: transaction} = await transactionApi
      .createTransactionHistory({
        platform: BlockChainPlatform.ETH,
        chainId: chainId!,
        tokenContractAddress: getAddress(USDC_ADDRESS.get(chainId)!),
